...
static void	handle_redirects(char *command, char **commands, t_env **env)
{
	char	redirect;
		...
		t_env **env)
{
	...
	if (is_builtin(args[0]))
		execute_forked_builtin(args, env);
	else
...
int	execute_multiple_commands(char **commands, t_env **env)
{
	int	original_fds[2];
	int	*children_pid;
	int	i;
	...
	while (commands[i])
	{
		...
		{
			free(children_pid);
			handle_redirects(commands[i], commands, env);
			execute_forked_command(commands[i], commands, env);
		}
		i++;
	}
