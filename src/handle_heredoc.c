...
void	read_heredoc(int *exit_status, t_env *env, char *delimiter,
		int heredoc_number)
{
	char	*line_read;
	char	*filename;
...
int	handle_heredoc(char *input, int *exit_status, t_env *env)
{
	...
	*heredoc_position = heredoc_number;
	...
	if (!exec_heredoc(delimiter, heredoc_number, exit_status, env))
	{
		free(delimiter);
		return (FAILED);
	}
	free(delimiter);
	return (handle_heredoc(input, exit_status, env));
}
